  ...
  try {
    ...
    if (privateKey.startsWith("{") && privateKey.endsWith("}")) {
      try {
        ...
        if (parsedJson.private_key) privateKey = parsedJson.private_key;
        if (parsedJson.client_email) clientEmail = parsedJson.client_email;
      } catch (je) {}
    }
    ...
    if (emailStr.startsWith("{") && emailStr.endsWith("}")) {
      try {
        ...
        if (parsedEmailJson.client_email) emailStr = parsedEmailJson.client_email;
        if (parsedEmailJson.private_key) privateKey = parsedEmailJson.private_key;
      } catch (je) {}
    }
...
setupUpstoxRoutes(app as any);
setupDhanRoutes(app as any);
setupAngelRoutes(app as any);
setupKiteRoutes(app as any);
...
app.get("/stock-price", async (req, res) => {
  try {
    ...
    try {
      ...
      if (!apiResponse.ok) throw new Error(`query2 failed`);
    } catch (e) {
      usedQuery = 'query1';
...
app.get("/mf-nav", async (req, res) => {
  try {
    ...
    if (!apiResponse.ok) return res.status(apiResponse.status).json({ error: "MFAPI error" });
...
app.get("/stock-search", async (req, res) => {
  try {
    ...
    if (!response.ok) return res.status(response.status).json({ error: "Failed to search Yahoo Finance" });

    const data: any = await response.json();
    const quotes = data?.quotes || [];

    const suggestions = quotes
      .filter((q: any) => q.symbol && (q.quoteType === 'EQUITY' || q.quoteType === 'ETF') && (q.symbol.endsWith('.NS') || q.symbol.endsWith('.BO') || q.exchange === 'NSI' || q.exchange === 'BSE'))
      .map((q: any) => {
    ...
    const finalSuggestions = suggestions.length > 0 ? suggestions : quotes.slice(0, 5).map((q: any) => ({
